...
export const DrugImage: React.FC<DrugImageProps> = ({
  ...
  if (imageError) {
    // SVG fallback with authentic chemical formula and glowing badge
    return (
      <div
        className={`${sizeDimensions[size]} rounded-xl bg-gradient-to-br from-slate-900 to-slate-950 border border-emerald-800/70 flex flex-col items-center justify-center p-1 relative overflow-hidden shadow-inner shrink-0 ${className}`}
      ...
      >
        ...
        <Atom className="w-5 h-5 text-emerald-400 opacity-80 mb-0.5" />
  ...
  return (
    <div
      className={`${sizeDimensions[size]} rounded-xl bg-slate-900/80 border border-slate-800 flex items-center justify-center overflow-hidden shrink-0 relative shadow-md ${className}`}
    >
      <img
        src={imageSrc}
        alt={drug.name}
